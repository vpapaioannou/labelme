...
- [x] Given a class "Ignore" to denote all regions that should be ignored during some processing X, e.g. Deep Learning training, ensure that this class will always be shown always last accross images into the files generated by `labelme_json_to_dataset` command. If every image has the same amount of "non - Ignore" classes, i.e. that should be taken in account in processing X, e.g. Deep Learning training, then the same color will annotated to the "Ignore" class.
- [x] Generated files by `labelme_json_to_dataset` have the same name as the input while maintaining the corresponding file extensions. For example, if input file is A then the output will be files A.png, A_label.png and A_label_viz.png.
